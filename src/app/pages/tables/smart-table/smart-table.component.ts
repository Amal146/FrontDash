...
export class SmartTableComponent implements OnInit {
  ...
  severityOptions = ["Low", "Medium", "High", "Critical"];
   ...
   getApplicationList() {
    ...
    return this.applicationOptions ? this.applicationOptions.map(app => ({ value: app, title: app })) : [];
  }
    ...
    columns: {
      ...
      status: {
        ...
        editor: {
          type: 'list',
          config: {
            list: this.statusOptions.map(option => ({ value: option, title: option })),
          },
        },
      },
      severity: {
        ...
        editor: {
          type: 'list',
          config: {
            list: this.severityOptions.map(option => ({ value: option, title: option })),
          },
        },
      },
      reportedAt: {
  ...
  onCreateConfirm(event): void {
    ...
    this.incidentService.createIncident(newIncident).subscribe((createdIncident: Incident) => {
